...
cdef int** new_int_ptr_array(int size):
    cdef int** array = <int**>malloc(size * sizeof(int*))
    return array
...
cdef Triples* new_triples():
    ...
    triples.all_subjects = new_int_ptr_array(100)
    triples.all_predicates = new_int_ptr_array(100)
    triples.all_objects = new_int_ptr_array(100)
...
cdef void resize_triples(Triples* triples):
    size = triples.limit
    if triples.all_n == triples.limit:
        new_size = triples.limit + 100
        triples.all_subjects = resize_array(triples.all_subjects, size, new_size)
        triples.all_predicates = resize_array(triples.all_predicates, size, new_size)
        triples.all_objects = resize_array(triples.all_objects, size, new_size)
        triples.limit = new_size
    triples.all_subjects[triples.all_n] = triples.subjects
    triples.all_predicates[triples.all_n] = triples.predicates
    triples.all_objects[triples.all_n] = triples.objects
    triples.subjects = new_int_array(ARRAY_SIZE)
    triples.predicates = new_int_array(ARRAY_SIZE)
    triples.objects = new_int_array(ARRAY_SIZE)
    triples.all_n += 1
...
#     cdef char* value = NULL
    ...
    return triples.all_n * 1000 + triples.n
